...
class StackViewViewModel: ObservableObject {
        ...
        func isTopCard(_ cardViewData: RevisionData) -> Bool {
        ...
        func moveCards(_ isForward: Bool) {
            if isForward {
                ...
                let data = StackViewViewModel.datas[lastCardIndex % StackViewViewModel.datas.count]
                cardViewDatas.append(data)
            } else {
                cardViewDatas.removeLast()
                self.lastCardIndex -= 1
                let data = StackViewViewModel.datas[(lastCardIndex - 2) % StackViewViewModel.datas.count]
        ...
        private var maxID: Int {
            return StackViewViewModel.datas.map { $0.id }.max() ?? 0
        }
        
        func rotateSingleLeft(_ chars : [RevisionData]) -> [RevisionData] {
            let first = chars[0]
            var arr = chars
        ...
        func rotateSingleRight(_ chars : [RevisionData]) -> [RevisionData] {
            let last = chars.last
            var arr = chars
